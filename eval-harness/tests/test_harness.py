...
from harness import RegistroSetPatron, SetPatron, cargar_set_patron, correr_arnes


def test_correr_arnes_calcula_exactitud_sobre_predicciones_mixtas():
    set_patron = SetPatron(
        version="v-test",
        registros=[
    ...
    assert boleta.version == "v-test"


def test_cargar_set_patron_lee_fixture_json(tmp_path: Path):
    ruta = tmp_path / "set.json"
    ruta.write_text(
        '{"version": "v1", "registros": [{"id": "a", "entrada": "x", "referencia": "x"}]}',
        encoding="utf-8",
    )

    set_patron = cargar_set_patron(ruta)

    assert set_patron == SetPatron(
        version="v1", registros=[RegistroSetPatron(id="a", entrada="x", referencia="x")]
    )
...
def test_componente_ficticio_corre_de_punta_a_punta_contra_la_fixture():
    ...
    assert boleta.total == len(set_patron.registros)
    assert 0.0 < boleta.exactitud < 1.0
    assert boleta.version == set_patron.version


def test_boletas_de_la_misma_version_de_set_patron_son_comparables():
    set_patron = SetPatron(
        version="v-comparable",
        registros=[RegistroSetPatron(id="a", entrada="x", referencia="x")],
    )
    ...
    boleta_corrida_1 = correr_arnes(set_patron, ComponenteFijo())
    boleta_corrida_2 = correr_arnes(set_patron, ComponenteFijo())

    assert boleta_corrida_1.version == boleta_corrida_2.version == "v-comparable"
